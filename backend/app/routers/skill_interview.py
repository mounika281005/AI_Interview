...
@router.post(
    ...
    response_model=SkillInterviewResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Start resume-based interview",
    description="Generate personalized interview questions based on user's resume."
)
async def start_resume_interview(
    ...
    1. Validates user has resume data
    2. Creates interview session
    3. Generates personalized questions based on resume
    4. Returns session with questions
    """
    debug_log("=== START RESUME INTERVIEW ===")
    debug_log("Request received", {
        "num_questions": request.num_questions,
        "difficulty": request.difficulty,
        "user_id": current_user.id
    ...
    # Always reload latest user row from DB to avoid stale in-memory auth object
    user_result = await db.execute(
        select(User).where(User.id == current_user.id)
    )
    latest_user = user_result.scalar_one_or_none()
    if not latest_user:
        raise HTTPException(
    ...
    resume_data = latest_user.resume_data

    # If parsed resume JSON is missing but resume file exists, auto-parse on demand.
    # This avoids forcing users to re-upload the same resume just to start interview.
    if not resume_data and latest_user.resume_url:
        try:
            from app.services.resume_service import ResumeParsingService
            resume_file_path = latest_user.resume_url.lstrip("/")
            if os.path.exists(resume_file_path):
                ...
                latest_user.resume_data = parsed
                await db.commit()
                await db.refresh(latest_user)
                resume_data = latest_user.resume_data
        except Exception as e:
            logger.warning(f"On-demand resume parsing failed: {e}")

    if not resume_data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No parsed resume data found. Please upload your resume in Profile."
        )

    # Create session
    target_role = latest_user.job_role or "Software Engineer"
    new_session = InterviewSession(
        user_id=latest_user.id,
        job_role=target_role,
        interview_type="resume_based",
        skills_tested=resume_data.get("extracted_skills", [])[:10],
        experience_level=latest_user.experience_level or "mid",
        difficulty=request.difficulty,
        total_questions=request.num_questions,
        time_limit_per_question=120,
        status=InterviewStatus.CREATED.value
    ...
    debug_log("Generating resume-based questions...")
    question_service = QuestionGeneratorService()
    generation_started = time.time()

    try:
        # Use categories from request if provided, otherwise default to technical-heavy mix
        resume_categories = getattr(request, 'categories', None) or ["technical", "behavioral", "situational"]
        generated_questions = await question_service.generate_resume_based_questions(
        ...
        logger.warning(f"Resume-based question generation failed: {e}, using fallback")
        debug_log("Resume generation failed, using standard questions", str(e))
        # Fallback to standard questions with resume skills
        generated_questions = await question_service.generate_questions(
            role=target_role,
            skills=resume_data.get("extracted_skills", [])[:5],
            experience_years=resume_data.get("total_years_experience", 3),
            difficulty=request.difficulty,
            num_questions=request.num_questions,
    ...
    for i, q in enumerate(generated_questions, start=1):
        ...
        q_keywords = getattr(q, 'expected_topics', []) if hasattr(q, 'expected_topics') else (q.get('expected_keywords', []) if isinstance(q, dict) else [])
        q_ideal = getattr(q, 'ideal_answer', None) if hasattr(q, 'ideal_answer') else (q.get('ideal_answer') if isinstance(q, dict) else None)
        q_category = getattr(q, 'category', 'behavioral') if hasattr(q, 'category') else (q.get('category', 'behavioral') if isinstance(q, dict) else 'behavioral')
    ...
    return SkillInterviewResponse(
        session_id=new_session.id,
        technology="Resume-Based Interview",
        difficulty=request.difficulty,
        total_questions=request.num_questions,
        questions=skill_questions,
        created_at=new_session.created_at
...
@router.post(
    "/{session_id}/submit-text/{question_id}",
    summary="Submit text answer for a question",
    description="Submit a typed text answer for a specific question."
)
async def submit_text_answer(
    ...
    """
    Submit a typed text answer for a question.

    Sets the transcript directly from the typed text so evaluation
    can proceed without STT transcription.
    """
    debug_log("=== SUBMIT TEXT ANSWER ===")
    debug_log("Input params", {
        "session_id": session_id,
        "question_id": question_id,
        "user_id": current_user.id,
        "text_length": len(body.get("answer_text", ""))
    })
    ...
    # Set transcript directly from typed text
    question.transcript = answer_text
    question.transcript_confidence = 1.0  # Perfect confidence for typed text
    question.answered_at = datetime.now(timezone.utc)

    await db.commit()
    debug_log("Text answer submitted successfully")
...
@router.post(
    "/{session_id}/adaptive-followup/{question_id}",
    summary="Generate adaptive follow-up question",
    description="Generate a technical follow-up question based on the previous answer for resume-based interviews."
)
async def generate_adaptive_followup(
    ...
    """
    Create one adaptive technical follow-up question from the previous answer context.
    """
    result = await db.execute(
    ...
    if session.interview_type != "resume_based":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Adaptive follow-ups are available only for resume-based interviews"
        )
    ...
    base_question = question_result.scalar_one_or_none()
    ...
    if followup_count >= 3:
        return {
            "success": True,
            "message": "Adaptive follow-up limit reached",
            "question": None
        }

    # Resolve answer text from explicit payload, transcript, or audio transcription
    answer_text = ((body or {}).get("answer_text") or base_question.transcript or "").strip()
    if not answer_text and base_question.audio_file_path:
        try:
            stt_service = SpeechToTextService()
            tr = await stt_service.transcribe(base_question.audio_file_path, language="en")
            answer_text = (tr.text or "").strip()
            if answer_text:
                base_question.transcript = answer_text
                base_question.transcript_confidence = tr.confidence
        except Exception as e:
            logger.warning(f"Adaptive follow-up transcription failed: {e}")

    if not answer_text:
        return {
            "success": False,
            "message": "No answer available to generate adaptive follow-up",
            "question": None
        }

    question_service = QuestionGeneratorService()
    resume_data = current_user.resume_data or {}
    adaptive_q = await question_service.generate_adaptive_followup_question(
        original_question=base_question.question_text,
    ...
    max_order = max((q.question_order for q in session.questions), default=0)
    new_question = InterviewQuestion(
        session_id=session.id,
        question_text=adaptive_q.question_text,
        question_order=max_order + 1,
        category="technical",
        difficulty=session.difficulty or "medium",
        expected_keywords=adaptive_q.expected_topics,
        ideal_answer=adaptive_q.ideal_answer,
        time_limit=adaptive_q.time_limit
    )
    db.add(new_question)
    await db.flush()
    ...
    return {
        "success": True,
        "message": "Adaptive follow-up generated",
        "question": SkillQuestion(
            id=new_question.id,
            question_number=new_question.question_order,
            question_text=new_question.question_text,
            technology="Resume-Based",
            difficulty=new_question.difficulty,
            expected_keywords=new_question.expected_keywords or [],
            ideal_answer=new_question.ideal_answer,
            time_limit_seconds=new_question.time_limit
        ).model_dump()
    }
...
async def submit_interview(
    ...
    # Check if answers are provided (audio or text)
    answered_questions = [q for q in questions if q.audio_file_path or (q.transcript and q.transcript.strip())]
    ...
    if not answered_questions:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No answers recorded. Please record or type answers before submitting."
        )
    ...
    # Minimum word count to consider a transcript as valid speech
    ...
    for i, question in enumerate(questions):
        ...
        # Step 1: Check if text answer already exists (typed answer)
        if question.transcript and question.transcript.strip():
            debug_log("Using existing text answer (typed)", {
                "length": len(question.transcript),
                "preview": question.transcript[:100]
            })
            transcript = question.transcript.strip()
        # Step 1b: Transcribe audio (STT service now includes hallucination filtering)
        elif question.audio_file_path and os.path.exists(question.audio_file_path):
            ...
            if stt_service:
                try:
                    ...
                    # After sanitization by STT service, empty text means no valid speech
                    transcript = transcription_result.text.strip() if transcription_result.text else ""
                    question.transcript_confidence = transcription_result.confidence
                    debug_log("Transcription result", {
                        "length": len(transcript),
                        "word_count": len(transcript.split()) if transcript else 0,
                        "confidence": transcription_result.confidence,
                        "preview": transcript[:100] if transcript else "(empty)"
                    })
                except Exception as e:
                ...
                debug_log("No STT service available")
                transcript = ""
        else:
            debug_log("No audio file or text answer", question.audio_file_path)
            transcript = ""

        # Step 2: Validate transcript before evaluation
        transcript_word_count = len(transcript.split()) if transcript else 0
        has_valid_speech = bool(transcript and transcript_word_count >= MIN_TRANSCRIPT_WORDS)

        # Store transcript consistently — use actual text or empty string (never fake text)
        question.transcript = transcript if transcript else ""

        # Log transcript for debugging BEFORE evaluation
        logger.info(f"[Q{i+1}] Question: {question.question_text[:80]}...")
        logger.info(f"[Q{i+1}] Transcript ({transcript_word_count} words): {transcript[:200] if transcript else '(empty)'}")
        logger.info(f"[Q{i+1}] Valid speech: {has_valid_speech}")

        # Step 3: Evaluate answer — ONLY if real speech was detected
        if has_valid_speech and evaluation_service:
            debug_log("Evaluating answer with real transcript...")
            try:
                ...
                structure_score = min(5.0, eval_result.relevance.score / 20)
                ...
                overall_score = 0
                strengths = []
                improvements = ["Evaluation service encountered an error"]
        else:
            # No valid speech detected — score is 0
            debug_log("No valid speech detected — assigning score 0")
            grammar_score = 0
            fluency_score = 0
            structure_score = 0
            similarity_score = 0
            ...
            strengths = []
            if not transcript:
                improvements = ["No response detected — please record your answer"]
            elif transcript_word_count < MIN_TRANSCRIPT_WORDS:
                improvements = ["Response too short to evaluate — please provide a more detailed answer"]
            else:
    ...
    session.status = InterviewStatus.EVALUATED.value  # Override to EVALUATED
