...
@router.get(
    "/timing-analytics",
    response_model=TimingAnalyticsResponse,
    summary="Get timing analytics",
    description="Get average question generation, evaluation, and total interview durations."
)
async def get_timing_analytics(
    ...
    """
    Returns real measured timing metrics for interview runtime behavior.
    """
    ...
    if not sessions:
        return TimingAnalyticsResponse(
            ...
            sessions_analyzed=0,
            avg_question_generation_ms=0.0,
            avg_evaluation_ms=0.0,
            avg_total_interview_seconds=0.0,
            by_question_count=[],
    ...
    for s in sessions:
        ...
        gen_ms = settings.get("question_generation_ms")
        eval_ms = settings.get("evaluation_total_ms")
        total_sec = s.duration_seconds or 0
        q_count = s.total_questions or 0

        if isinstance(gen_ms, (int, float)):
            generation_values.append(float(gen_ms))
        if isinstance(eval_ms, (int, float)):
            evaluation_values.append(float(eval_ms))
        if isinstance(total_sec, (int, float)):
            total_values.append(float(total_sec))

        if q_count not in buckets:
            buckets[q_count] = {
                "sessions": 0,
                "gen": [],
                "eval": [],
        ...
        buckets[q_count]["sessions"] += 1
        if isinstance(gen_ms, (int, float)):
            buckets[q_count]["gen"].append(float(gen_ms))
        if isinstance(eval_ms, (int, float)):
            buckets[q_count]["eval"].append(float(eval_ms))
        if isinstance(total_sec, (int, float)):
            buckets[q_count]["total"].append(float(total_sec))

    def _avg(values):
        return round(sum(values) / len(values), 2) if values else 0.0

    by_question_count = []
    for q_count in sorted(buckets.keys()):
        b = buckets[q_count]
        by_question_count.append(
            TimingByQuestionCount(
                question_count=q_count,
                sessions=b["sessions"],
                avg_generation_ms=_avg(b["gen"]),
                avg_evaluation_ms=_avg(b["eval"]),
                avg_total_interview_seconds=_avg(b["total"]),
            )
        )
    ...
    return TimingAnalyticsResponse(
        ...
        avg_question_generation_ms=_avg(generation_values),
        avg_evaluation_ms=_avg(evaluation_values),
        avg_total_interview_seconds=_avg(total_values),
...
async def calculate_scores(
    ...
    **Returns:**
    ...
    # Calculate scores from individual question evaluations
    ...
    if not evaluated_questions:
        raise HTTPException(
            ...
            detail="No evaluated questions found. Please evaluate questions first."
        )

    # Calculate average scores across all evaluated questions
    n = len(evaluated_questions)
    avg_relevance = sum(q.relevance_score or 0 for q in evaluated_questions) / n
    avg_grammar = sum(q.grammar_score or 0 for q in evaluated_questions) / n
    avg_fluency = sum(q.fluency_score or 0 for q in evaluated_questions) / n
    avg_keyword = sum(q.keyword_score or 0 for q in evaluated_questions) / n

    # Calculate weighted total score
    breakdown = scoring_service.calculate_final_score(
    ...
    question_overall = [q.overall_score or 0 for q in evaluated_questions]
    avg_time = sum(q.time_taken_seconds or 0 for q in evaluated_questions) / n

    # Update session with scores
    session.overall_score = breakdown.total_score
    session.relevance_score = breakdown.relevance_score
    session.grammar_score = breakdown.grammar_score
    session.fluency_score = breakdown.fluency_score
    session.technical_score = breakdown.keyword_score
    session.grade = breakdown.letter_grade
    session.status = InterviewStatus.EVALUATED.value

    # Update user statistics
    current_user.update_statistics(breakdown.total_score)
    ...
    section_scores = [
        SectionScore(
            ...
            weight=0.35,
            weighted_score=breakdown.weighted_relevance,
            feedback=_get_section_feedback("relevance", breakdown.relevance_score)
        ),
        SectionScore(
            section_name="Grammar",
            score=breakdown.grammar_score,
            weight=0.20,
            weighted_score=breakdown.weighted_grammar,
            feedback=_get_section_feedback("grammar", breakdown.grammar_score)
        ),
        SectionScore(
            section_name="Fluency",
            score=breakdown.fluency_score,
            weight=0.25,
            weighted_score=breakdown.weighted_fluency,
            feedback=_get_section_feedback("fluency", breakdown.fluency_score)
        ),
        SectionScore(
            section_name="Technical/Keywords",
            score=breakdown.keyword_score,
            weight=0.20,
            weighted_score=breakdown.weighted_keyword,
            feedback=_get_section_feedback("keywords", breakdown.keyword_score)
        ),
    ]
    ...
    return ScoringResponse(
        ...
        questions_skipped=len(session.questions) - n,
        best_question_score=max(question_overall) if question_overall else 0,
        worst_question_score=min(question_overall) if question_overall else 0,
...
async def generate_feedback(
    ...
    **Returns:** Comprehensive feedback report
    ...
    # Build question results from evaluated questions
    question_results = []
    for q in session.questions:
        if q.is_evaluated:
            question_results.append({
                ...
                'fluency_score': q.fluency_score or 0,
                'keyword_score': q.keyword_score or 0,
                'overall_score': q.overall_score or 0,
            })

    # Generate feedback using service (sync method, no await)
    feedback_service = FeedbackService()
    feedback_result = feedback_service.generate_feedback(
        session_id=session_id,
        overall_score=session.overall_score,
        metric_scores={
            'relevance': session.relevance_score or 0,
            'grammar': session.grammar_score or 0,
            'fluency': session.fluency_score or 0,
            'keywords': session.technical_score or 0,
        },
        question_results=question_results,
        role=session.job_role,
    )

    # Map service result to dict format
    strengths_list = [
        {"area": s.category, "description": s.message, "examples": s.examples}
        for s in feedback_result.strengths
    ]
    weaknesses_list = [
        {"area": w.category, "description": w.message,
         "impact": "high" if w.priority <= 1 else ("medium" if w.priority <= 2 else "low")}
        for w in feedback_result.weaknesses
    ]
    suggestions_list = [
        {"suggestion": s.message, "category": s.category, "priority": s.priority}
        for s in feedback_result.suggestions
    ]
    resources_list = [
        {"title": r.title, "type": r.type, "url": r.url or "", "description": r.description}
        for r in feedback_result.resources
    ]
    ...
    feedback_record = InterviewFeedback(
        session_id=session_id,
        user_id=current_user.id,
        performance_rating=feedback_result.overall_rating,
        executive_summary=feedback_result.summary,
        strengths=strengths_list,
        weaknesses=weaknesses_list,
        improvement_suggestions=suggestions_list,
        job_readiness_score=feedback_result.readiness_score,
        readiness_level=feedback_result.readiness_level,
        improvement_percentage=None,
        performance_trend=None,
        recommended_resources=resources_list,
        practice_topics=feedback_result.next_steps
    )
    ...
    session.strengths = [s["area"] for s in strengths_list]
    session.weaknesses = [w["area"] for w in weaknesses_list]
    session.suggestions = [s["suggestion"] for s in suggestions_list]
    session.summary = feedback_result.summary
    ...
    return FeedbackResponse(
        ...
        grade=session.grade,
        strengths=[StrengthItem(**s) for s in strengths_list],
        weaknesses=[WeaknessItem(**w) for w in weaknesses_list],
        suggestions=[SuggestionItem(**s) for s in suggestions_list],
        best_answer=None,
        worst_answer=None,
        job_readiness_score=feedback_result.readiness_score,
        readiness_level=feedback_result.readiness_level,
...
@router.get(
    ...
    description="Get existing feedback for an interview session."
)
async def get_session_feedback(
    ...
    """
    Get existing feedback for an interview session.

    **Path Parameters:**
    - **session_id**: Interview session ID

    **Returns:** Feedback data if available
    """
    # Get session
    result = await db.execute(
        select(InterviewSession)
    ...
    feedback_record = feedback_result.scalar_one_or_none()

    if not feedback_record:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Feedback not generated yet. Please generate feedback first."
        )

    return FeedbackResponse(
        success=True,
        session_id=session_id,
        performance_rating=feedback_record.performance_rating,
        executive_summary=feedback_record.executive_summary,
        total_score=session.overall_score,
        grade=session.grade,
        strengths=[StrengthItem(**s) if isinstance(s, dict) else StrengthItem(area=str(s), description="", examples=[]) for s in (feedback_record.strengths or [])],
        weaknesses=[WeaknessItem(**w) if isinstance(w, dict) else WeaknessItem(area=str(w), description="", impact="medium") for w in (feedback_record.weaknesses or [])],
        suggestions=[SuggestionItem(**s) if isinstance(s, dict) else SuggestionItem(suggestion=str(s), category="general", priority=3) for s in (feedback_record.improvement_suggestions or [])],
        best_answer=None,
        worst_answer=None,
        job_readiness_score=feedback_record.job_readiness_score,
        readiness_level=feedback_record.readiness_level,
        estimated_practice_needed=0,
        improvement_percentage=feedback_record.improvement_percentage,
        performance_trend=feedback_record.performance_trend,
        recommended_resources=feedback_record.recommended_resources or [],
        practice_topics=feedback_record.practice_topics or [],
        generated_at=feedback_record.created_at
    )
...
@router.get(
    ...
    description="Get existing scores for an interview session."
)
async def get_session_scores(
    ...
    """
    Get existing scores for an interview session.
    ...
    **Returns:** Score breakdown
    ...
    # Build section scores using correct SectionScore schema fields
    section_scores = []
    if session.relevance_score is not None:
        section_scores.append(SectionScore(
            section_name="Relevance",
            score=session.relevance_score,
            weight=0.35,
            weighted_score=session.relevance_score * 0.35,
            feedback=_get_section_feedback("relevance", session.relevance_score)
        ))
    if session.grammar_score is not None:
        section_scores.append(SectionScore(
            section_name="Grammar",
            score=session.grammar_score,
            weight=0.20,
            weighted_score=session.grammar_score * 0.20,
            feedback=_get_section_feedback("grammar", session.grammar_score)
        ))
    if session.fluency_score is not None:
        section_scores.append(SectionScore(
            section_name="Fluency",
            score=session.fluency_score,
            weight=0.25,
            weighted_score=session.fluency_score * 0.25,
            feedback=_get_section_feedback("fluency", session.fluency_score)
        ))
    if session.technical_score is not None:
        section_scores.append(SectionScore(
            section_name="Technical/Keywords",
            score=session.technical_score,
            weight=0.20,
            weighted_score=session.technical_score * 0.20,
            feedback=_get_section_feedback("keywords", session.technical_score)
        ))

    # Calculate question-level stats
    questions_answered = sum(1 for q in session.questions if q.transcript)
    questions_skipped = len(session.questions) - questions_answered

    # Calculate best/worst question scores from direct question fields
    question_scores = [q.overall_score for q in session.questions if q.overall_score is not None]

    best_question_score = max(question_scores) if question_scores else 0.0
    worst_question_score = min(question_scores) if question_scores else 0.0

    # Calculate time stats
    total_time_seconds = session.duration_seconds or 0
    average_time_per_question = (
        total_time_seconds / len(session.questions)
    ...
    return ScoringResponse(
        ...
        section_scores=section_scores,
        total_score=session.overall_score,
        grade=session.grade or "F",
        percentile=None,
        questions_answered=questions_answered,
        questions_skipped=questions_skipped,
        best_question_score=best_question_score,
        worst_question_score=worst_question_score,
        total_time_seconds=total_time_seconds,
        average_time_per_question=average_time_per_question,
        calculated_at=session.updated_at or session.created_at
    )
...
async def get_dashboard_stats(
    ...
    **Returns:**
    ...
    # Calculate current streak (consecutive days with at least one interview)
    current_streak = 0
    if sessions:
        # Group sessions by date
        dates_with_interviews = set()
        for s in sessions:
            date_only = s.created_at.date()
            dates_with_interviews.add(date_only)

        # Count consecutive days from today backwards
        check_date = now.date()
        while check_date in dates_with_interviews:
            current_streak += 1
            check_date = check_date - timedelta(days=1)

    # Calculate total practice minutes (more granular than hours for dashboard)
    total_minutes = total_seconds / 60

    return DashboardStatsResponse(
        user_id=current_user.id,
        total_interviews=total_interviews,
        total_sessions=total_interviews,  # Alias for consistency
        completed_interviews=completed_interviews,
        average_score=round(average_score, 2),
        best_score=round(best_score, 2),
        total_practice_hours=round(total_hours, 1),
        total_practice_minutes=round(total_minutes, 0),  # Add minutes field
        ...
        current_streak=current_streak,  # Now calculated!
