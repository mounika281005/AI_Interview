...
class InterviewHistoryItem(BaseModel):
    ...
    session_id: str
    id: Optional[str] = None  # Alias for frontend compatibility
    job_role: str
    interview_type: str
    total_score: Optional[float] = None
    overall_score: Optional[float] = None  # Alias for frontend compatibility
...
class DashboardStatsResponse(BaseModel):
    ...
    # Overview stats
    total_interviews: int
    total_sessions: int = 0  # Alias for frontend compatibility
    completed_interviews: int
    average_score: float
    best_score: float
    total_practice_hours: float
    total_practice_minutes: float = 0  # More granular for dashboard display
...
class TimingByQuestionCount(BaseModel):
    """Timing metrics bucketed by selected question count."""
    question_count: int
    sessions: int
    avg_generation_ms: float
    avg_evaluation_ms: float
    avg_total_interview_seconds: float


class TimingAnalyticsResponse(BaseModel):
    """Schema for timing analytics used in review/performance reporting."""
    user_id: str
    sessions_analyzed: int
    avg_question_generation_ms: float
    avg_evaluation_ms: float
    avg_total_interview_seconds: float
    by_question_count: List[TimingByQuestionCount]
    generated_at: datetime
