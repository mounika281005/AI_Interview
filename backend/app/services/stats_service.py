...
class StatsService:
    ...
    async def get_user_stats(
        ...
        total_time = sum(s.duration_seconds or 0 for s in sessions) // 60
...
async def calculate_user_stats(
    ...
    return {
        ...
        "completed_interviews": stats.total_interviews,
        "average_score": stats.average_score,
        "best_score": stats.best_score,
        "improvement_trend": stats.improvement_rate,
        "strengths": [
            name for name, score in stats.skills_breakdown.items() if score >= 70
        ][:3],
        "areas_to_improve": [
            name for name, score in stats.skills_breakdown.items() if score < 60
        ][:3],
        "interview_history_summary": {
            "total_questions_answered": stats.total_questions_answered,
            "recent_score": stats.recent_score,
            "total_practice_time_minutes": stats.total_practice_time,
