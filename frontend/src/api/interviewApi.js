...
export const createSession = async (sessionData) => {
    try {
        ...
        const payload = {
            ...
            categories: sessionData.categories || null,
        };
...
export const completeSession = async (sessionId) => {
    try {
        const response = await apiClient.patch(ENDPOINTS.INTERVIEWS.COMPLETE_SESSION(sessionId));
        return {
            success: true,
            data: response.data.data || response.data,
        };
...
export const submitTextAnswer = async (sessionId, questionId, answerText) => {
    const url = ENDPOINTS.INTERVIEWS.SUBMIT_TEXT_ANSWER(sessionId, questionId);
    try {
        const response = await apiClient.post(url, { answer_text: answerText });
        return {
            success: true,
            data: response.data.data || response.data,
        };
...
export const uploadAudio = async (sessionId, questionId, audioFile, duration) => {
    ...
    try {
        // Determine file extension from MIME type (Blobs from MediaRecorder have no filename)
        const mimeToExt = {
            'audio/webm': 'webm',
            'audio/ogg': 'ogg',
            'audio/wav': 'wav',
        ...
        // Build FormData with a proper filename so the backend can validate the extension
        ...
        const response = await apiClient.post(
            ...
            formData,
            {
                ...
                // Could add upload progress tracking here if needed
            }
        );
