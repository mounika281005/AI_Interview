...
function HistoryPage() {
    ...
    // Helper to get score from either field name
    const getScore = (s) => s.overall_score || s.total_score || 0;
    const getId = (s) => s.id || s.session_id;
    const isCompleted = (s) => ['completed', 'evaluated'].includes(s.status);

    // Filter sessions
    const filteredSessions = sessions.filter(session => {
        if (filter === 'all') return true;
        if (filter === 'completed') return isCompleted(session);
        if (filter === 'in-progress') return !isCompleted(session);
        return true;
    });
            ...
            {sortedSessions.length > 0 ? (
                <div className="sessions-grid">
                    {sortedSessions.map(session => {
                        ...
                        return (
                            <div key={sessionId} className="history-card">
                                <div className="card-header">
                                    <h3>{session.title || `${role} Interview`}</h3>
                                    <div className="card-header-right">
                                        <span className={`status-badge ${session.status}`}>
                                            {session.status}
                                        </span>
                                        {isCompleted(session) && score > 0 && (
                                            <span className={`header-score ${getScoreClass(score)}`}>
                                                {Math.round(score)}%
                                            </span>
                                        )}
                                    </div>
                                ...
                                <div className="card-actions">
                                    {isCompleted(session) ? (
                                        <>
                                            ...
                                            <Link
                                                to={`/skill-interview/results/${sessionId}`}
