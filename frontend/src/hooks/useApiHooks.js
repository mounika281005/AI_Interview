...
export function useAudioRecording() {
    ...
    // Helper: kill the mic stream immediately
    ...
    const startRecording = useCallback(async () => {
        try {
            ...
            recorder.onstop = () => {
                ...
                // Always kill the stream when recording stops
                killStream();
            };
    ...
    const stopRecording = useCallback(() => {
        ...
        if (recorder && recorder.state !== 'inactive') {
            ...
            // Recorder already inactive — just kill the stream in case
            killStream();
            setIsRecording(false);
        }
    }, [killStream]);
    ...
    const loadDashboard = useCallback(async () => {
        ...
        try {
            ...
            if (historyResult.success) {
                ...
                const sessionsList = historyData?.interviews || historyData?.sessions || historyData?.items || (Array.isArray(historyData) ? historyData : []);
